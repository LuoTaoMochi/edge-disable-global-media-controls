...
$T = @{
    zh = @{
        ...
        Policy = '启动加速策略'
        RestartApps = '登录后重启应用'
        RestartAppsWarn = '警告：Windows“重新启动应用”处于开启状态。系统重启后自动恢复的 Edge 可能不带此参数，Global Media Controls 可能恢复。可加 -DisableRestartApps 关闭该功能。'
        Restarted = 'Edge 已重启'
    ...
    ja = @{
        ...
        Protocol = 'プロトコルコマンド'
        Shortcut = 'ショートカット'
        Policy = 'Startup Boost ポリシー'
        RestartApps = 'サインイン後のアプリ再起動'
        RestartAppsWarn = '警告：Windows の「アプリの再起動」が有効です。再起動後の Edge にパラメータが付かない場合があります。-DisableRestartApps で無効化できます。'
        Restarted = 'Edge を再起動しました'
        ExeNotFound = 'msedge.exe が見つかりません。手動で Edge を再起動してください'
        Done = '完了。'
    }
    en = @{
        Patched = 'Patched'
        ...
        Failed = 'Failed'
        Updated = 'Updated'
        NeedAdmin = '(administrator rights required)'
        MaybePerm = '(insufficient permissions?)'
        Startup = 'startup entry'
        Protocol = 'protocol command'
        Shortcut = 'shortcut'
        Policy = 'startup boost policy'
        RestartApps = 'restart apps after sign-in'
        RestartAppsWarn = 'Warning: Windows "restart apps after sign-in" is ON. A restored Edge process may not carry this flag. Pass -DisableRestartApps to turn it off.'
        Restarted = 'Edge restarted'
...
function Remove-OurFlag([string]$v) {
    ...
    # IMPORTANT:
    # This script only removes the exact standalone flag that it owns.
    # It deliberately does NOT parse, merge, or rewrite any other
    # --disable-features=... argument, so it cannot interfere with
    # MSCMonster/edge-no-rounded-corners.
...
function Add-OurFlag([string]$v) {
    ...
    # This guarantees compatibility with the rounded-corner project.
...
function Get-ActionText([string]$old) {
    ...
    if ($old -match '(?<!\S)--disable-features=GlobalMediaControls(?!\S)') {
        return $T.Updated
    }
    ...
    foreach ($d in $dirs) {
        foreach ($f in (Get-ChildItem $d -Filter *.lnk -Recurse -ErrorAction SilentlyContinue)) {
            try {
                ...
                if ($lnk.TargetPath -notlike '*msedge.exe') {
                    continue
                }
    ...
    try {
        foreach ($name in ((Get-Item $rk).GetValueNames() | Where-Object { $_ -like 'MicrosoftEdgeAutoLaunch*' })) {
            ...
            try {
                ...
                "$($T.Failed) $($T.MaybePerm): $($T.Startup) $name"
            }
        }
    }
    catch {
...
if ($RestartEdge) {
    ...
    if ($procs) {
        ...
        while (
            (Get-Process msedge -ErrorAction SilentlyContinue) -and
    ...
    if ($edgeExe -and (Test-Path $edgeExe)) {
        if ($Undo) {
            Start-Process $edgeExe -ArgumentList '--restore-last-session'
        }
        else {
            Start-Process $edgeExe -ArgumentList "$flag --restore-last-session"
        }
